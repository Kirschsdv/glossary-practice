...
- All JS alert/string literals containing Afrikaans text must use **double quotes** — `'n` inside a single-quoted JS string crashes the whole script.
...
- **Do not remove or modify the tool-switcher nav bar** (`.tool-switcher` div between `.app-header` and `.page-tabbar`). Leave it exactly as-is.
- Run a JS syntax check mentally before finishing: verify no unclosed strings, especially in Afrikaans text.

**Term data format — History style (use when subject has clear exam weighting):**
```js
const XTERMS = [
  {
...
**EMS style (use when terms are more evenly weighted):**
```js
const XTERMS = [
  {
    t: 'Topic Name',
...
- After adding a subject, upload the new `index.html` to your GitHub repository — GitHub Pages redeploys within ~30 seconds
